...
static async Task<int> ExecuteCheckAsync(ProgramOptions options)
{
    ...
    {
        var marker = diagnostic.Severity == GlshDiagnosticSeverity.Error ? "error" : "warning";
...
static async Task<int> ExecuteEmitAsync(ProgramOptions options)
{
    ...
    var outputDirectory = options.OutputDirectory ??
        Path.Combine(Path.GetDirectoryName(options.ProjectPath) ?? Environment.CurrentDirectory, "obj", "GLSH");
...
{
    Console.WriteLine("dotnet glsh <check|emit|build> <project>");
