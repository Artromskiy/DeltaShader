...
vec3 delta_transformPoint(mat4 matrix, vec3 point) { vec4 value = matrix * vec4(point, 1.0); if (value.w == 0.0) { return value.xyz; } return value.xyz / value.w; }
...
void main()
{
    ...
    {return;}
    vec3 local_result = delta_transformPoint(Input0_instance.data[local_index], Input1_instance.data_0[local_index]);
