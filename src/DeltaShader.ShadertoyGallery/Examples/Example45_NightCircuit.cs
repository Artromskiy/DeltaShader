...
{
    [FragmentShader]
    public static float4 NightCircuit(in GalleryFragmentContext context)
    {
        ...
        var pulseA = 0.5f + 0.5f * maths.sin(p.x * 17f - context.Constants.Time * 2.3f);
        var pulseB = 0.5f + 0.5f * maths.cos(p.y * 19f + context.Constants.Time * 1.7f);
        var nodes = 0f;
        ...
        return new float4(0.01f + trace * 0.05f, 0.025f + trace * 0.18f, 0.08f + trace * 0.45f, 1f);
    }
}
