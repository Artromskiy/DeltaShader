# ADR-0005: Symbol-identity based intrinsic registry for KibiHex and GLSH intrinsics

## Context
Shader lowering must avoid brittle string matching for `KibiHex` types and methods.
...
- `KibiHex.float2/3/4`, `KibiHex.int2/3/4`, `KibiHex.uint2/3/4`,
  `KibiHex.bool2/3/4` type symbols;
- vector constructors, user-defined operators and swizzle properties;
- selected `KibiHex.maths` methods (`sin`, `cos`, `tan`, `dot`, `normalize`).
