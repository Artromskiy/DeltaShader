...
| Texture derivatives beyond `ddx`/`ddy` | `intrinsics.fwidth`, `intrinsics.ddx` and `intrinsics.ddy` are stage-gated owned intrinsics; texture LOD control remains outside the current graphics slice | Add stage-gated texture derivative/LOD intrinsics and external validator coverage |
