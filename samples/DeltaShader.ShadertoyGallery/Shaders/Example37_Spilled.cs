...
{
    [FragmentShader]
    public static float4 Spilled(in GalleryFragmentContext context, in GalleryVarying input)
    {
        ...
        for (var drop = 0f; drop < 5f; drop += 1f)
        {
            ...
            var center = new float2(-0.58f + drop * 0.28f + 0.06f * maths.sin(context.Constants.Time * 0.4f + phase), 0.2f * maths.cos(phase * 2f) - 0.12f);
            var delta = p - center;
