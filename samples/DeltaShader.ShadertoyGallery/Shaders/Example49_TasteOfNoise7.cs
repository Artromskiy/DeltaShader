...
{
    [FragmentShader]
    public static float4 TasteOfNoise7(in GalleryFragmentContext context, in GalleryVarying input)
    {
        ...
        {
            var phase = maths.dot(p, new float2(3.1f + layer * 1.7f, 5.2f - layer * 0.63f)) + context.Constants.Time * (0.18f + layer * 0.04f);
            value += weight * (0.5f + 0.5f * maths.sin(phase + maths.cos(phase * 1.73f)));
            p = p * 1.92f + new float2(0.17f, -0.11f);
            weight = weight * 0.53f;
        }
        return new float4(0.04f + value * 0.22f, 0.06f + value * 0.38f, 0.12f + value * 0.62f, 1f);
    }
}
