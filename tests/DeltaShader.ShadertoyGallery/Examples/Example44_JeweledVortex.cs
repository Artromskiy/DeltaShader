...
{
    [FragmentShader]
    public static void JeweledVortex(
        ...
        [FragmentColor] out float4 color)
    {
        ...
        {
            var phase = angle * (7f + facet) + radius * (10f + facet * 2f) - constants.Time * (0.5f + facet * 0.17f);
            facets += (0.5f + 0.5f * maths.cos(phase)) * maths.exp(-radius * (1.4f + facet * 0.3f)) / (facet + 1f);
        }
        var jewel = maths.exp(-maths.abs(radius - 0.22f) * 38f);
        color = new float4(0.07f + facets * 0.22f + jewel * 0.18f, 0.02f + facets * 0.08f + jewel * 0.35f, 0.18f + facets * 0.48f + jewel * 0.56f, 1f);
    }
}
