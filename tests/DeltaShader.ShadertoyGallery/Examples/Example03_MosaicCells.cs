...
{
    [FragmentShader]
    public static float4 MosaicCells(in GalleryFragmentContext context)
    {
        ...
        var pulse = 0.5f + 0.5f * maths.sin(context.Constants.Time + maths.floor(tile.x) * 1.7f + maths.floor(tile.y) * 0.9f);
        return new float4(0.03f + 0.45f * window * pulse, 0.06f + 0.3f * window, 0.14f + 0.65f * window * (1f - pulse), 1f);
    }
}
