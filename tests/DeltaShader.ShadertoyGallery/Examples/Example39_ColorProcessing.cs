...
internal static class Example39_ColorProcessing
{
        ...
        [FragmentColor] out float4 color)
    {
        ...
        var green = 0.5f + 0.5f * maths.sin(p.y * 5f - constants.Time * 0.7f + 2.1f);
        var blue = 0.5f + 0.5f * maths.sin((p.x + p.y) * 3f + constants.Time * 0.4f + 4.2f);
        ...
        var contrast = 0.72f + 0.28f * maths.cos(maths.length(p) * 5f - constants.Time);
        color = new float4(processed.x * contrast, processed.y * contrast, processed.z * contrast, 1f);
    }
}
