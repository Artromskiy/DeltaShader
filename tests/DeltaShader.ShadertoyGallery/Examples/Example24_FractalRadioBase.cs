...
{
    [FragmentShader]
    public static void FractalRadioBase(
        ...
        [FragmentColor] out float4 color)
    {
        ...
        for (var band = 0f; band < 5f; band += 1f)
        {
            ...
            var wave = 0.5f + 0.5f * maths.cos(radius * frequency - constants.Time * (1.1f + band * 0.14f) + angle * (band + 1f));
            waves += wave * (0.55f / (band + 1f));
        }
        var halo = maths.exp(-radius * radius * 2.4f);
        color = new float4(0.03f + waves * 0.24f + halo * 0.12f, 0.07f + waves * 0.35f, 0.18f + waves * 0.5f + halo * 0.35f, 1f);
    }
}
