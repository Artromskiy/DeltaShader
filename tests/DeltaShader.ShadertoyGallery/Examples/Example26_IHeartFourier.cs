...
{
    [FragmentShader]
    public static void IHeartFourier(
        ...
        [FragmentColor] out float4 color)
    {
        ...
        var outline = 0.43f + 0.1f * maths.cos(angle * 2f) - 0.04f * maths.cos(angle * 4f) + 0.025f * maths.sin(angle * 7f + constants.Time);
        var fill = 1f - maths.smoothStep(outline - 0.018f, outline + 0.018f, radius);
        var rim = maths.exp(-maths.abs(radius - outline) * 55f);
        var pulse = 0.75f + 0.25f * maths.sin(constants.Time * 3f);
        color = new float4(0.35f + fill * 0.5f + rim * 0.2f, 0.025f + fill * 0.07f, 0.06f + fill * 0.12f + rim * 0.28f, 1f) * pulse;
    }
}
